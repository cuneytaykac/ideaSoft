...
class Navigation{
  ...
  static Future<T?>? push<T>({required Widget page, PageRouteBuilder<T>? transition}) {
    if (transition == null) {
  ...
  static Future<T?>? pushAndRemoveAll<T>({required Widget page, PageRouteBuilder<T>? transition}) {
    if (transition == null) {
      return navigatorKey.currentState?.pushAndRemoveUntil(_materialPageRoute(page), (route) => false);
    } else {
      return navigatorKey.currentState?.pushAndRemoveUntil(transition, (route) => false);
    }
  }
